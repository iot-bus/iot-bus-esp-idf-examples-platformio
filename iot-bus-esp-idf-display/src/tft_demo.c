...
static void disp_images() {
	...
	if (spiffs_is_mounted) {
		...
		TFT_jpg_image(CENTER, CENTER, 3, "/images/test1.jpg", NULL, 0);
		Wait(500);

		TFT_jpg_image(CENTER, CENTER, 2, "/images/test2.jpg", NULL, 0);
		Wait(500);

		TFT_jpg_image(CENTER, CENTER, 1, "/images/test4.jpg", NULL, 0);
		Wait(500);

		// ** Show full size JPG image
		tstart = clock();
		TFT_jpg_image(CENTER, CENTER, 0, "/images/test3.jpg", NULL, 0);
		tstart = clock() - tstart;
		...
		for (int scale=5; scale >= 0; scale--) {
			tstart = clock();
			TFT_bmp_image(CENTER, CENTER, scale, "/images/tiger.bmp", NULL, 0);
			tstart = clock() - tstart;
